  ...
  //////////*Personalizado: *////////
   ...
   // Adiciona "R$" no saldo disponível
   const saldoElements = document.getElementsByClassName("valorReais");
   for (let i = 0; i < saldoElements.length; i++) {
       let saldo = saldoElements[i].textContent.trim();
       saldoElements[i].textContent = "R$ " + saldo;
   }


////* PoupUps na página Gerencial.php *///// 

     // PoupUps Editar e Excluir usuários
  ...
  document.querySelectorAll('.excluir-btn').forEach(function(button) {
      button.addEventListener('click', function() {
          excluirUsuario(button);
      });
  });

  // Fechar popups
  document.getElementById('closePopupEditar').addEventListener('click', function() {
      document.getElementById('popupEditar').style.display = 'none';
  });

  document.getElementById('closePopupExcluir').addEventListener('click', function() {
